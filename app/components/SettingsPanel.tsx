  ...
  return (
    <div className="w-full lg:w-[480px] animate-in slide-in-from-left-5 duration-700 ease-out">
      <Card className="h-full rounded-2xl overflow-hidden border border-slate-200/70 shadow-md bg-white/95 backdrop-blur-sm transition-all duration-300 hover:shadow-lg hover:border-slate-300/70">
        ...
        <CardBody className="relative h-[calc(100vh-180px)]">
          <div className="absolute inset-0 overflow-y-auto scrollbar-none">
            <form id="settings-form" onSubmit={handleSubmit} className="flex flex-col min-h-full p-6">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-sm border-t border-slate-200/50 z-10">
            <Button
              type={isRunning ? "button" : "submit"}
              form="settings-form"
              onPress={isRunning ? handleStop : undefined}
              color={isRunning ? "danger" : "primary"}
              className={`w-full font-medium shadow-md transition-all duration-300 ease-out rounded-xl py-6 ${
                isRunning 
